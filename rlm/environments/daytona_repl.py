...
def _build_exec_script(
    ...
    return build_broker_exec_script(code, broker_port, depth, custom_tools_code)
...
class DaytonaREPL(IsolatedEnv):
    ...
    def setup(self):
        ...
        self.sandbox.fs.upload_file(
            build_broker_script(self.BROKER_PORT).encode("utf-8"),
            "broker_server.py",
        )
    ...
    def execute_code(self, code: str) -> REPLResult:
        ...
        stdout = result_text if response.exit_code == 0 else ""
        stderr = f"Exit code {response.exit_code}: {result_text}" if response.exit_code != 0 else ""
